...
```
Using the code given in the appendix, a simulated population of size `r ntot` was randomly generated from the exponential distribution, with lambda equal to `r lambda`. The distribution of this population is shown in Figure 1, with the theoretical mean indicated by the vertical red line positioned at the value `r exp.mean`. The calculated mean of the sample population was `r pop.mean`, which is quite close to the theoretical value of `r exp.mean`. Likewise, the calculated standard deviation of the population, `r pop.sd`, was also very close to the theoretical value of `r exp.sd`.
